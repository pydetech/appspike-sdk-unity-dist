...
/// <summary>
/// AppSpike Remote Config usage with an on-screen IMGUI panel: setup → fetch controls →
/// fetch settings → custom signals → typed getters → fetch info, plus a second screen
/// listing every key/value with its source and a prefix filter. Create an empty GameObject
/// in any scene, attach this component, and paste your API key into the Api Key field in
/// the Inspector.
/// </summary>
public class RemoteConfigSample : MonoBehaviour
{
    ...
    // The one place the API key lives. It is an edit-time Inspector field, not a runtime
    // text box: a shipping game passes its key at build time, so the sample models that.
    [SerializeField]
    ...
    private void DrawMainScreen()
    {
        ...
        GUILayout.Label("AppSpike Remote Config Sample", _headerStyle);
        ...
        if (_initialized)
        {
            ...
            GUILayout.Label("GetString(welcome_message) = " + _remoteConfig.GetString("welcome_message"), _labelStyle);
            GUILayout.Label("GetBoolean(feature_enabled) = " + _remoteConfig.GetBoolean("feature_enabled"), _labelStyle);
            GUILayout.Label("GetLong(max_retries) = " + _remoteConfig.GetLong("max_retries"), _labelStyle);
            GUILayout.Label("GetDouble(price_multiplier) = " + _remoteConfig.GetDouble("price_multiplier"), _labelStyle);
            ...
            GUILayout.Label("GetValue(binary_payload).ByteArrayValue = " + BitConverter.ToString(payload), _labelStyle);

            GUILayout.Space(10);
            GUILayout.Label("Fetch info", _headerStyle);
            var info = _remoteConfig.Info;
            GUILayout.Label("last fetch: " + info.LastFetchStatus + " at " + info.FetchTime + " UTC", _labelStyle);
    ...
    private void DrawFetchSettingsSection()
    {
        ...
        // ConfigSettings reads back whatever SetConfigSettingsAsync last applied; the pair
    ...
    private void DrawCustomSignalsSection()
    {
        ...
        // Free-form entry: any key and any string value, so every custom_signal condition
        // in the published template can be exercised from the running sample.
    ...
    private void DrawAllValuesScreen()
    {
        ...
        if (keys.Count == 0)
        {
            GUILayout.Label("No values — set defaults or fetch and activate first.", _labelStyle);
        }
    ...
    private async System.Threading.Tasks.Task InitializeAsync()
    {
        if (apiKey.Length == 0 || apiKey == PlaceholderApiKey)
        {
            _status = "No API key: the Api Key field is still \"" + PlaceholderApiKey +
                ...
                "field in the Inspector (serialized by Assets/RemoteConfigSample.cs), " +
        ...
        if (result.IsError)
        {
            _status = "Initialize failed: " + ((InitResult.Error)result).Message;
            return;
        }
        ...
        _initialized = true;
        _status = "Initialized. Fetch to load the published template.";
    }

    /// <summary>
    /// The sample's in-app defaults: served until a fetched template is activated, and as
    ...
    private async System.Threading.Tasks.Task ResetAsync()
    {
        ...
        // settings; the sample re-registers its defaults straight away, so the all-values
        // screen shows the defaults set with (default) sources and no remote rows.
        _remoteConfig.Reset();
        await SetSampleDefaultsAsync();
    ...
    private async System.Threading.Tasks.Task ApplySignalAsync(string key, string value)
    {
        ...
        // A signal only changes the evaluated template at the next non-throttled fetch,
        // so bypass the minimum fetch interval and activate right away.
